...
2. **모드 선택**에서 **L4 라우팅**을 선택하고 **확인**을 클릭하세요. IP와 포트로 부하를 분산하는 방식으로, 웹 서버를 여러 대로 늘리는 이 구성에는 L4로 충분합니다.
...
2. **로드 밸런서 생성**을 클릭하세요. 목록에 나타나기까지 12분 걸리며, 생성되면 사설 IP가 할당됩니다.
...
2. **이미지**에서 웹 서버를 실행할 이미지를 선택합니다. 애플리케이션까지 포함해 미리 만들어 둔 개인 이미지를 쓰면 증설된 인스턴스의 기동 시간이 짧아집니다.
3. **루트 블록 스토리지**에서 타입 **HDD**, 크기 `20`GB를 지정합니다. 선택한 이미지의 최소 블록 스토리지보다 작게 지정할 수 없습니다.
4. **기본 정보 설정**의 **인스턴스 정보**를 입력합니다.

- **가용성 영역**: **임의의 가용성 영역**을 선택합니다. 인스턴스마다 영역이 개별적으로 정해지며, 균등 배치를 보장하지는 않으므로 모든 인스턴스가 한 영역에 몰릴 수도 있습니다.
...
1. **추가 설정**의 **네트워크**에서 **사용 가능한 서브넷**의 `mysubnet2 (192.168.0.0/24)`를 클릭합니다. 로드 밸런서와 같은 서브넷이어야 합니다.
...
사용자 스크립트는 인스턴스가 처음 부팅될 때 root 권한으로 한 번 실행됩니다. 첫 줄의 `#!/bin/bash`는 이 파일을 bash로 실행하라는 표시로, 반드시 첫 줄에 있어야 합니다.

2번 블록이 증설을 확인하기 위한 부하입니다. 갓 만들어진 인스턴스에는 부하를 줄 트래픽도 접속 수단도 없으므로, 인스턴스가 부팅하면서 스스로 부하를 만들게 합니다. 증설로 새로 생긴 인스턴스에도 같은 스크립트가 실행되므로 그룹 평균이 계속 높게 유지됩니다.

- `$(nproc)`: 이 인스턴스의 vCPU 개수입니다. vCPU마다 하나씩 띄워야 CPU 전체가 찹니다.
- `while :; do :; done`: 아무 일도 하지 않고 무한 반복하는 루프로, vCPU 하나를 100%로 채웁니다.
- `timeout 900s`: 900초 뒤 루프를 자동으로 종료합니다. 부하가 풀리면서 감축 조건이 충족됩니다. **부팅 시간 + 증설 지속 시간 + 증설된 인스턴스의 부팅 시간**보다 길게 잡습니다.
- `setsid nohup ... &`: 스크립트가 끝난 뒤에도 프로세스가 살아 있도록 백그라운드로 분리합니다.
...
> [!NOTE]
> 왼쪽 드롭다운은 `instance`로 고정되어 있어 선택할 항목이 없습니다.

1. **인스턴스 조정**에 `1`, **재사용 대기 시간**에 `5`(분)을 입력합니다. 재사용 대기 시간은 인스턴스를 생성한 뒤 추가 생성을 시도하지 않는 시간입니다. 새 인스턴스가 부팅되고 트래픽을 나눠 받기까지 걸리는 시간보다 짧게 잡으면 증설 효과가 나타나기 전에 증설이 연달아 일어납니다.
2. **감축 정책**에 임계값 `30`(%), 지속 시간 `5`(분), 인스턴스 조정 `1`개, 재사용 대기 시간 `5`분을 입력합니다. 감축의 재사용 대기 시간은 인스턴스를 **삭제한 뒤 추가 삭제를 시도하지 않는** 시간입니다.

> [!WARNING]
> 증설 임계값과 감축 임계값을 가깝게 지정하면 증설과 감축이 번갈아 반복됩니다. 증설 70%, 감축 60%로 두면 인스턴스가 늘어 사용률이 65%로 떨어진 뒤 다시 오르내리며 생성과 삭제가 되풀이됩니다. 두 임계값 사이를 충분히 벌리세요.

1. **자동 복구 정책**에서 **사용 안 함**을 선택합니다. 운영 환경에서는 **사용**을 선택해 장애가 난 인스턴스를 자동으로 대체하지만, 자동 복구는 성능 지표가 3분 동안 수집되지 않으면 장애로 판단합니다. 인스턴스 수는 그대로인데 목록의 인스턴스 ID만 바뀌어 증설과 혼동되므로 확인하는 동안에는 꺼 둡니다.
2. **로드 밸런서**의 **사용 가능한 로드 밸런서**에서 `lb-autoscale`을 클릭해 **선택된 로드 밸런서**로 옮기세요. 이 목록에는 앞에서 선택한 서브넷과 같은 서브넷을 사용하는 로드 밸런서만 표시됩니다. 목록이 비어 있다면 서브넷 선택을 다시 확인하세요.
3. **스케일링 그룹 생성**을 클릭하세요. **구동 인스턴스**에 입력한 수만큼 인스턴스가 자동으로 생성되고, 그 시점부터 인스턴스 요금이 부과됩니다.
...
1. 증설을 확인합니다. 사용자 스크립트가 부하를 만들고 있으므로 기다리기만 하면 됩니다. 인스턴스 부팅이 끝난 시점부터 증설 조건의 지속 시간인 5분이 지나면 **인스턴스 목록**에 새 인스턴스 ID가 추가됩니다.

**통계** 탭에서 CPU 사용률이 임계값을 넘은 구간과 증설 시점이 맞는지 대조할 수 있습니다. 조회 기간은 최근 24시간으로 고정되어 있습니다.

1. 감축을 확인합니다. `timeout`이 만료되면 부하가 자동으로 풀립니다. 증설된 인스턴스는 나중에 부팅됐으므로 부하도 나중에 풀립니다. 모든 인스턴스의 부하가 풀린 뒤 감축 조건의 지속 시간이 지나면 인스턴스가 줄어들며, **최소 인스턴스까지만** 줄어듭니다.

이 가이드의 값으로 구성했을 때 실제로 관찰된 흐름입니다.


| 경과  | 상태                                             |
| --- | ---------------------------------------------- |
| 0분  | 스케일링 그룹 생성. 구동 인스턴스 2대 생성 시작                   |
| 4분  | 2대 `CREATE_COMPLETE`. 사용자 스크립트 실행, CPU 100% 시작 |
| 10분 | **증설.** 3번째 인스턴스 생성                            |
| 12분 | 3번째 인스턴스 완료. 멤버 3대 모두 `ACTIVE`                 |
| 18분 | 처음 2대의 부하 자동 해제                                |
| 27분 | 3번째 인스턴스의 부하 자동 해제                             |
| 34분 | **감축.** 3대에서 2대로 감소 후 정지                       |


감축 시에는 가장 먼저 생성된 인스턴스가 삭제되었습니다.
